...
LR = 1e-4
SEED = 42
TRAINABLE_BASE_LAYERS = 10  # how many base layers to unfreeze from the end

DATASET_ROOT = Path("dataset/dataset_224x224")
TRAIN_DIR = DATASET_ROOT / "train"
VAL_DIR = DATASET_ROOT / "val"
MODEL_SAVE_PATH = "midjourney_vs_dalle_mobilenet_detector.keras"
...
# ---------- Model: MobileNetV2 backbone + classifier ----------

def build_mobilenet_model(img_size=IMG_SIZE, num_classes=NUM_CLASSES, trainable_base_layers=0, lr=LR, label_smoothing=0.0):
    """
    Build transfer learning model using MobileNetV2 as backbone.
...
def train_model(model, train_generator, val_generator, epochs=EPOCHS):
    """
    Train the MobileNetV2 model with callbacks and class weights.
    """
    if model is None:
        raise ValueError("Model not built. Call build_mobilenet_model() first.")
...
def evaluate_model(model, val_generator):
    ...
    plt.tight_layout()
    plt.savefig("mobilenet_confusion_matrix.png", dpi=150)
    plt.show()
...
def sanity_check_and_visualize(train_dir):
    ...
    plt.tight_layout()
    plt.savefig("dataset_comparison_mobilenet.png", dpi=150, bbox_inches="tight")
    plt.show()

    print("\nComparison saved to 'dataset_comparison_mobilenet.png'")
...
def run_pipeline(
    ...
    """Run the full MobileNetV2 pipeline end-to-end.
    ...
    print(f"Using MobileNetV2 with image size {img_size}x{img_size}")
    ...
    plot_training_history(history, title_prefix="MOBILENET")

    # Evaluate on validation set
    print("\nEvaluating MOBILENET model on VALIDATION set...")
