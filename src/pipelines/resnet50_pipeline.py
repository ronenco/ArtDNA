...
LR = 1e-4
SEED = 42
TRAINABLE_BASE_LAYERS = 15  # how many base layers to unfreeze from the end

DATASET_ROOT = Path("dataset/dataset_224x224")
TRAIN_DIR = DATASET_ROOT / "train"
VAL_DIR = DATASET_ROOT / "val"
MODEL_SAVE_PATH = "midjourney_vs_dalle_resnet_detector.keras"
...
# ---------- Model: ResNet50 backbone + classifier ----------

def build_resnet_model(img_size=IMG_SIZE, num_classes=NUM_CLASSES, trainable_base_layers=0, lr=LR, label_smoothing=0.0):
    """
    Build transfer learning model using ResNet50 as backbone.
    ...
    # Preprocessing for ResNet50 (caffe-style: RGB to BGR, zero-center)
...
def train_model(model, train_generator, val_generator, epochs=EPOCHS):
    """
    Train the ResNet50 model with callbacks and class weights.
    """
    if model is None:
        raise ValueError("Model not built. Call build_resnet_model() first.")
...
def sanity_check_and_visualize(train_dir):
    ...
    plt.tight_layout()
    plt.savefig("dataset_comparison_resnet.png", dpi=150, bbox_inches="tight")
    plt.show()

    print("\nComparison saved to 'dataset_comparison_resnet.png'")
...
def run_pipeline(
    ...
    """Run the full ResNet50 pipeline end-to-end.
    ...
    print(f"Using ResNet50 with image size {img_size}x{img_size}")
    ...
    plot_training_history(history, title_prefix="RESNET")
